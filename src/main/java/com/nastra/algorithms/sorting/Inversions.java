 ...
 * the number of inverted pairs of indices.' <br>
 * <br>
 * If a list is already sorted, then the number of inversions is 0. If the list
 * is sorted in reverse order, then the number of inversions is at its maximum.
 * The number of possible inversions can be expressed by n * (n - 1) / 2 where n
 * is the length of the input list. Formally speaking, two elements a[i] and
 * a[j] form an inversion if a[i] > a[j] and i < j. For example, the input [2,
 * 4, 1, 3, 5] has three inversions (2, 1), (4, 1), (4, 3).
 * 
 * <br>
 * So in order to solve the problem, we basically can use merge countInversions.
 * The strategy is as following: When there is an element a[j] that is larger
 * than a[i], we know that the rest of the elements residing in the array from
 * i..mid are also inversions. Therefore we simply take (mid + 1 - i) and sum it
 * up to the inversions already found so far. The time complexity is O(n log n)
 * for sorting and counting the inversions.
 * 
